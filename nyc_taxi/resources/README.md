...
The databricks CLI bundles top level is defined by file `nyc_taxi/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`nyc_taxi/resources/model-workflow-resource.yml`)
 - batch inference workflow (`nyc_taxi/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`nyc_taxi/resources/monitoring-resource.yml`)
 - feature engineering workflow (`nyc_taxi/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`nyc_taxi/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `nyc_taxi/databricks.yml` and `nyc_taxi/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-nyc_taxi-model     | /dev-nyc_taxi-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-nyc_taxi-model | /staging-nyc_taxi-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-nyc_taxi-model    | /prod-nyc_taxi-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-nyc_taxi-model    | /test-nyc_taxi-experiment    |
...
2. Complete the `TODO` in `nyc_taxi/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `nyc_taxi/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `nyc_taxi/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-nyc_taxi-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-nyc_taxi-batch-inference-job`
that runs the notebook under `nyc_taxi/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-nyc_taxi-batch-inference-job`.
...
`test-nyc_taxi-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-nyc_taxi-batch-inference-job`.
